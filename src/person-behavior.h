    ...
    if (no_mates > 0 && person->sex_ == Sex::kMale &&
      ...
      for (int i = 0; i < no_mates; i++) {
        ...
        if (mate->age_ > env->GetMaxAge() + 1) {
          std::cout << "I am a male aged " << person->age_
                    << " and my casual partner is aged " << mate->age_
                    << " months" << std::endl;
        }
...
struct RegularMatingBehaviour : public Behavior {
  ...
  void Run(Agent* agent) override {
    ...
    //   std::cout << "An acute male, aged " << person->age_ << " months is in
    //   partnership with a female aged " << person->partner_->age_ << "months."
    //   << std::endl;
    // }
    // if (person->sex_==Sex::kFemale) {
    //   std::cout << "An acute female, aged " << person->age_ << " months is in
    //   partnership with a male aged " << person->partner_->age_ << "months."
    //   << std::endl;
    // }
    //  }
    //  if (person->partner_ == nullptr) {
    // if (person->sex_==Sex::kMale) {
    //  std::cout << "An acute male, aged " << person->age_ << " months is not
    //  in a regular partnership." << std::endl;
    //}
    // if (person->sex_==Sex::kFemale) {
    //  std::cout << "An acute female, aged " << person->age_ << " months is not
    //  in a regular partnership." << std::endl;
    //}
    //  }
    //}

    // if (person->hasPartner() && (person->state_==GemsState::kAcute ||
    // person->state_==GemsState::kChronic)) {
    //     std::cout << "An infected male aged " << person->age_ << "is in
    //     partnership with a female aged" << person->partner_->age_ << " " <<
    //     std::endl;
    // }
    // if (person->hasPartner() && (person->partner_->state_==GemsState::kAcute
    // || person->partner_->state_==GemsState::kChronic)) {
    //     std::cout << "A male aged " << person->age_ << "is in partnership
    //     with an infected female aged" << person->partner_->age_ << " " <<
    //     std::endl;
    // }
    // if (person->hasPartner() && person->state_==GemsState::kHealthy &&
    // person->partner_->state_==GemsState::kHealthy) {
    //     std::cout << "A healthy male aged " << person->age_ << "is in
    //     partnership with a healthy female aged" << person->partner_->age_ <<
    //     " " << std::endl;
    // }
